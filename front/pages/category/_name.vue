<template>
    <section class="container">
        <div class="all-list-header">
            <div class="list-header">当前分类：{{categoryName}}</div>
        </div>
...
export default {
    ...
    validate ({ params, store }) {
        //校验 `params.name ` 是否存在
        var is = false
        store.state.menuData.forEach(element => {
            element.children.forEach(element1 => {
                if(element1.name == params.name){
                    is = true
                }
            });
        });
    ...
    methods:{
        currentChange(val){
            this.pageNo = val
            this.$axios.get('/front/category_ArticesByPage',{params:{pageNo:this.pageNo,pageSize:this.pageSize,name:this.categoryName}}).then(res => {
    ...
    async asyncData (context) {
        ...
        let[res2] = await Promise.all([
            // 获取文章数据
            context.$axios.get(`/front/category_ArticesByPage`,{params:{pageNo:1,pageSize:10,name:context.params.name}}).then(res => {
                return res
            })
        ])
        return{
